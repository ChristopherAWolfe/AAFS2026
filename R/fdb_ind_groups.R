# Script 2 - FDB: Condensed Groups x Sex
...
## Data Prep
dat <- read.csv("data/FDB_Fall2025.csv")

dat_sub <- dat %>% filter(Race %in% c("Black", "Hispanic", "White")) %>%
  select(Sex, Race, GOL, XCB, ZYB, BBH, BNL, MAB, AUB, NPH, FMB, ASB, ZMB, NLH, 
         NLB, OBB, OBH, EKB, DKB, FRC, PAC, OCC, FOL, MDH)

male <- dat_sub %>% filter(Sex == "M") %>% na.omit()
female <- dat_sub %>% filter(Sex == "F") %>% na.omit()
all <- dat_sub %>% na.omit()
...
mod_FRC <- cmdstan_model("code/norm_IT_FRC.stan")
...
for(i in 1:ncol(male_vars)){
  ...
  standat <- list(N = nrow(male),
                  G = 3,
                  group = as.integer(as.factor(male$Race)),
                  y = male_vars[,i])
  ...
  fit$save_object(file = paste0("results/male_uni_fdb/",colnames(male_vars)[i], ".RDS"))
  rm(fit)
  gc()
...
male_mat[male_mat < 0] <- 0
write.csv(male_mat, "results/male_uni_fdb/summary_male_fdb.csv")
...
for(i in 1:ncol(female_vars)){
  ...
  standat <- list(N = nrow(female),
                  G = 3,
                  group = as.integer(as.factor(female$Race)),
                  y = female_vars[,i])
  print(colnames(female_vars[i]))
  fit <- mod_uni$sample(data = standat, refresh = 500, parallel_chains = 4, 
                        adapt_delta = 0.99, max_treedepth = 12)
  summ <- fit$summary(c("H_cond", "H_marg", "I_Y_G"))
  female_mat[i,1:4] <- c(colnames(female_vars)[i], summ[1,2], summ[2,2], summ[3,2])
  fit$save_object(file = paste0("results/female_uni_fdb/",colnames(female_vars)[i], ".RDS"))
  rm(fit)
  gc()
...
write.csv(female_mat, "results/female_uni_fdb/summary_female_fdb.csv")
...
it_metrics  %>% ggplot(aes(group, mi, fill = group)) + geom_col() + facet_wrap(vars(variable))
...
for(i in 1:ncol(all_vars)){
  ...
  standat <- list(N = nrow(all),
                  G = 3,
                  group = as.integer(as.factor(all$Race)),
                  y = all_vars[,i])
  
  fit <- mod_uni$sample(data = standat, refresh = 500, parallel_chains = 4)
  summ <- fit$summary(c("H_cond", "H_marg", "I_Y_G"))
  all_mat[i,1:4] <- c(colnames(all_vars)[i], summ[1,2], summ[2,2], summ[3,2])
  fit$save_object(file = paste0("results/combined_uni_fdb/",colnames(all_vars)[i], ".RDS"))
  rm(fit)
  gc()
...
write.csv(all_mat, "results/combined_uni_fdb/summary_combined_fdb.csv")

### Multivariate

all_scaled <- all %>%
  mutate(across(c(GOL, XCB, ZYB, BBH, BNL, MAB, AUB, NPH, 
                  FMB, ASB, ZMB, NLH, NLB, OBB, OBH, EKB, 
                  DKB, FRC, PAC, OCC, FOL, MDH), 
                scale))
...
fit_multi <- brm(bform, 
                 ...
                 chains = 4, 
                 backend = "cmdstanr", 
                 control = list(adapt_delta = 0.995,  # increase from 0.99
                                max_treedepth = 15))

saveRDS(fit_multi, "results/multi_fdb/multi_fdb.RDS")

post <- as_draws_df(fit_multi)
D <- length(fit_multi$ranef$resp)
G <- length(unique(fit_multi$data$Race))
draws <- nrow(post)

outcome_names <- fit_multi$ranef$resp

#### Get group names from posterior names
r_vars <- grep("^r_Race__", names(post), value = TRUE)
...
for (d in seq_len(D)) {
  ...
  for (g in seq_len(G)) {
    gname <- group_names[g]
    rname <- paste0("r_Race__", outcome, "[", gname, ",Intercept]")
...
pop_counts <- table(fit_multi$data$Race)
...
write.csv(multi_mat, "results/multi_fdb/summary_fdb.csv")
...
for(i in 1:ncol(all_vars)){
  ...
  dat3 <- as.data.frame(cbind(all$Race, all_vars[,i]))
...
train.control <- trainControl(method = "LOOCV")
# Train the model using rpart
model <- train(Race ~ GOL + XCB + ZYB + BBH + BNL + MAB + AUB + NPH + FMB +
                 ASB + ZMB + NLH + NLB + OBB + OBH + EKB + DKB + FRC + PAC + OCC +
                 FOL + MDH, data = all, method = "lda", preProcess = c("center", "scale"),trControl = train.control)
predictions <- predict(model, all)
cmat <- confusionMatrix(predictions, as.factor(all$Race))
cmat

multi_mat <- as.data.frame(multi_mat)
multi_mat[5,1] <- "Accuracy"
multi_mat[5,2] <- cmat$overall[1]
write.csv(multi_mat, "results/multi_fdb/summary_multi_fdb.csv")
